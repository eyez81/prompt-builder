...
const emptyForm={task:"",audience:"",context:"",material:"",format:"",length:"",tone:"",criteria:"",askQuestions:true,verifyFacts:true,avoidInventing:true};
const templates=[
 ...
 {label:"מחקר והשוואה",icon:"⌕",form:{task:"השווה בין",format:"טבלת השוואה ולאחריה מסקנה מנומקת",criteria:"הפרד בין עובדות, פרשנות ומסקנות; ציין אי־ודאות",verifyFacts:true}},
 {label:"כתיבה ועריכה",icon:"✎",form:{task:"ערוך ושפר את הטקסט הבא",format:"גרסה ערוכה ולאחריה רשימה קצרה של השינויים המרכזיים",tone:"טבעי, מדויק ובהיר",criteria:"שמור על המשמעות ועל הקול של הכותב"}}
];
...
const fieldIds=["task","audience","context","material","format","length","tone","criteria","askQuestions","verifyFacts","avoidInventing"];
...
function syncFields(){fieldIds.forEach(id=>{const el=byId(id);if(el.type==="checkbox")el.checked=Boolean(form[id]);else el.value=form[id]||""});renderPrompt()}
function bindFields(){fieldIds.forEach(id=>byId(id).addEventListener("input",e=>{form[id]=e.target.type==="checkbox"?e.target.checked:e.target.value;save();renderPrompt()}))}
...
function buildPrompt(){
 ...
 const rules=[form.askQuestions&&"אם חסר מידע חיוני, שאל עד 3 שאלות הבהרה לפני כתיבת התשובה.",form.verifyFacts&&"בדוק עובדות מרכזיות והבחן בבירור בין עובדה, הערכה ואי־ודאות.",form.avoidInventing&&"אל תמציא מידע. אם אינך יודע או שאין די מידע, אמור זאת במפורש."].filter(Boolean);if(rules.length)s.push(`כללי עבודה:\n${rules.map(x=>`• ${x}`).join("\n")}`);
 return s.join("\n\n");
}
...
function renderPrompt(){
 ...
 byId("copyButton").disabled=!prompt;
 const score=[Boolean(form.task.trim()),Boolean(form.audience.trim()||form.context.trim()),Boolean(form.format.trim()),Boolean(form.criteria.trim()),form.askQuestions||form.verifyFacts||form.avoidInventing].filter(Boolean).length*20;
 byId("score").textContent=score;byId("scoreRing").style.setProperty("--score",`${score*3.6}deg`);
 byId("scoreTitle").textContent=score>=80?"פרומפט חזק":score>=50?"כיוון טוב":"כדאי להוסיף פרטים";
 byId("scoreText").textContent=score>=80?"ברור, ממוקד וניתן לביצוע":"השלימו עוד שדות כדי לדייק את התוצאה";
}
...
function applyTemplate(template){form={...emptyForm,...template.form};activeStep=0;syncFields();save();renderStep();byId("task").focus()}
async function copyPrompt(){const text=buildPrompt();if(!text)return;try{await navigator.clipboard.writeText(text)}catch{const area=document.createElement("textarea");area.value=text;document.body.appendChild(area);area.select();document.execCommand("copy");area.remove()}const button=byId("copyButton"),old=button.innerHTML;button.innerHTML="<span>✓</span>הועתק!";setTimeout(()=>button.innerHTML=old,1800)}
...
function init(){
 ...
 ["טבלה השוואתית","שלבים ממוספרים","דוגמה מלאה","עברית פשוטה"].forEach(value=>{const button=document.createElement("button");button.textContent=value;button.addEventListener("click",()=>{form.format=form.format?`${form.format}, ${value}`:value;syncFields();save()});byId("suggestionChips").appendChild(button)});
